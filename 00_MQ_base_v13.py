"""
00_MQ_base_v13.py
Improved appearance of Resultscreen due to requests (end user testing)
"""
...
class Sidepanel(ctk.CTkFrame):
    def __init__(self, master):
        ...
        self.label = ctk.CTkLabel(self, text="Maori quiz", font=('Arial', 20, "bold"))
        ...
        self.appearance_mode_menu = ctk.CTkOptionMenu(
            self, values=["System", "Light", "Dark"], command=self.change_appearance_mode_event
        )
...
class QuizScreen(ctk.CTkFrame):
    def __init__(self, master):
        ...
        self.label1 = ctk.CTkLabel(self, text="", font=("Arial", 14))
        self.label1.grid(row=1, column=1, padx=20, pady=20)
        self.label2 = ctk.CTkLabel(self, text="")
        self.label2.grid(row=2, column=1)
        self.scorelabel = ctk.CTkLabel(self, text="Score: 0", font=("Arial", 15))
        self.scorelabel.grid(row=5, column=1)
        self.start_quiz_button = ctk.CTkButton(self, text="Start Quiz", font=("Arial", 14, "bold"), command=self.start_quiz)
        ...
        self.submit_button = ctk.CTkButton(self, text="Submit", font=("Arial", 14, "bold"), command=self.submit)
...
class WordScreen(ctk.CTkFrame):
    ...
    def update_words(self, category):
        ...
        words_label = ctk.CTkLabel(
            self.word_table, text=f"Words in {category}:", font=("Arial", 18)
        )
        words_label.grid(row=0, column=0, columnspan=2, pady=(0, 10))

        maori_label = ctk.CTkLabel(
            self.word_table, text="Maori words", font=("Arial", 14, "bold")
        )
        maori_label.grid(row=1, column=0, padx=10)
        english_label = ctk.CTkLabel(
            self.word_table, text="English words", font=("Arial", 14, "bold")
        )
...
class ResultsScreen(ctk.CTkFrame):
    ...
    def update_scores(self, category):
        ...
        scores_label = ctk.CTkLabel(
            self.result_table, text=f"Latest 10 Scores for {category}:", font=("Arial", 15)
        )
        ...
        for i, score in enumerate(scores_data, start=2):
            rank = ctk.CTkLabel(self.result_table, text=f"{i - 1}")
            rank.grid(row=i, column=0, padx=10, pady=1)
...
class ChoosableUI(ctk.CTkSegmentedButton):
    def __init__(self, master, word_screen, results_screen):
        ...
        self.word_choice = ctk.CTkSegmentedButton(master, values=self.choices, font=("Arial", 14))
...
class Quiz:
    ...
    def next_question(self):
        ...
        if self.current_question_index < len(self.questions):
            ...
            self.label1.configure(text="Quiz finished!", font=("Arial", 15, "bold"))
            self.submission_textbox.grid_forget()
            self.submit_button.grid_forget()
            self.start_quiz_button.grid(row=2, column=1, padx=20, pady=20)
    ...
    def show_question(self):
        ...
        self.label2.configure(text=f"Question: {question.english_word.title()}", font=("Arial", 15, "bold"))
        ...
        if self.current_question:
            if submission == self.current_question.maori_word:
                ...
                self.label1.configure(
                    text=f"Incorrect. The correct answer for {self.current_question.english_word} is {self.current_question.maori_word}."
                )
        ...
        with open(filename, 'r', encoding='utf-8') as file:
            ...
            for row in reader:
                ...
                self.questions.append(
                    Question(english_word.strip(), maori_word.strip(), category)
                )
